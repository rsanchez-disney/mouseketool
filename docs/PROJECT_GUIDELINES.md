...
### Versioning (Semver)
- All packages (`frontend`, `backend`, `desktop`, `playwright-tests`) share the same version number.
...
- When bumping versions, update all four `package.json` files to the same version.
...
3. **Write documentation updates** to the Help page markdown files in `docs/help/` and any relevant `.md` files (README, docs/). **Do not use emojis** in any documentation files. **Do not add project structure trees** or "What's new" / changelog sections to README files.
4. If the feature adds a new help section, create a new `.md` file in the appropriate `docs/help/<category>/` folder with proper frontmatter (id, tab, title, icon, order, optional group).
5. **Present a summary** of what was added/changed.
6. **Wait for the developer to review and approve the documentation** before committing.
7. **Only after docs approval**, commit the documentation changes as a separate commit.
8. **Only after both commits are on the branch**, merge to main.
...
Before writing or modifying any frontend code, AI agents and developers **must** read and follow:
- `docs/DESIGN_GUIDELINES.md` — visual identity, component library, color semantics, layout rules
- `docs/UI_DESIGN_PATTERNS.md` — exact markup patterns for cards, sheets, buttons, badges, tooltips, forms, toasts, and all reusable UI structures

### Enforcement
- Every new component or page must conform to the patterns documented in those files.
- Do not introduce new UI patterns, component libraries, or styling approaches without updating the design docs first.
- Every icon button must have a tooltip. Every card title must start with a Lucide icon. Every button must include `cursor-pointer`.
- Use the shared components (`Toggle`, `ConfirmDialog`, `FolderBrowser`, `LogViewer`) instead of reimplementing similar functionality.
- AI features must be gated behind the `kiroAvailable` inject — never show AI UI when Kiro is unavailable.
